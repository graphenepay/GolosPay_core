...
class Payment{
    ...
    {
        if(array_has($data, "payid")) {$this->payid = $data['payid'];} else {$this->payid = GolosHelper::generatePaymentID();};

        if(array_has($data, "callback")) {$this->callback = $data['callback'];} else {$this->callback = "http://www.Golospay.io";};
        //
