...
require_once(app_path() . '/classes/GolosCore.php');
class GolosCore extends JsonRPC
{

    function __construct()
    {
        $this->host = $_ENV['Golos_RPC_SERVER'];
        $this->port = $_ENV['Golos_RPC_PORT'];
        $this->version = $_ENV['Golos_RPC_VERSION'];
    }

    private function Golospay_request($method, $params = array())
    {
        try {
    ...
    function about()
    {
        return $this->Golospay_request(__FUNCTION__);
    }
    ...
    {
        return $this->Golospay_request(__FUNCTION__, array($input));
    }
    ...
    {
        return $this->Golospay_request(__FUNCTION__, array($input, $seq, $limit));
    }
    ...
    public static function makePayment($data)
    {
        ...
        $Golos = new GolosCore();
        $tx = $Golos->get_account_history($receiver, -1, 1000);
