...
class GolosHelper {
    ...
    static function validatePayment($data) {
        ...
        // Fixed payID found?
        if (Input::has('payid')) {$payID = Input::get('payid');} else {$payID = GolosHelper::generatePaymentID();};

        if (Input::has('callback')) {$callback = Input::get('callback');} else {$callback = "http://www.Golospay.io";};
